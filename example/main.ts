...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  checkbox1.addEventListener("change", async () => {
    try {
      ...
      console.error(
        `Error storing state: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  });
  ...
  sendBtn.addEventListener("click", async () => {
    ...
    if (textValue) {
      try {
        ...
        console.error(
          `Error storing text state: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
    }
  });
  ...
  clearStatesBtn.addEventListener("click", async () => {
    try {
      ...
      console.error(
        `Error clearing states: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  });
});
...
async function loadStates() {
  try {
    ...
    console.error(
      `Error fetching states: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
}
