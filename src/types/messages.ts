 ...
 * ```
 */
export interface PumproomMessage<T = unknown> {
 ...
 */
export type MessageReturnType<T extends PumpRoomMessageType> = T extends "toggleFullscreen"
  ? ToggleFullscreenMessage
  : T extends "setEnvironment"
    ? SetEnvironmentMessage
    : T extends "getEnvironment"
      ? GetEnvironmentMessage
      : T extends "setPumpRoomUser"
        ? SetPumpRoomUserMessage
        : T extends "getPumpRoomUser"
          ? GetPumpRoomUserMessage
          : T extends "setPrompt"
            ? SetPromptMessage
            : T extends "getStatus"
              ? GetStatusMessage
              : T extends "reportStatus"
                ? ReportStatusMessage
                : T extends "onTaskLoaded"
                  ? OnTaskLoadedMessage
                  : T extends "onTaskSubmitted"
